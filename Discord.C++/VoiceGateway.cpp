...
pplx::task<void> DiscordCPP::VoiceGateway::identify() {
	return pplx::create_task([this] {
		...
		if (_resume == true) {
			...
			this->send(out_payload).then([this] {
...
void DiscordCPP::VoiceGateway::on_websocket_incoming_message(const web::json::value& payload) {
	...
	case 6:
		...
		break;
	case 8:
		_heartbeat_interval = (int)(payload.at(U("d")).at(U("heartbeat_interval")).as_integer() * 0.75);
		identify();
		break;
	case 9:
		_reconnect_timeout = 0;
...
DiscordCPP::VoiceGateway::VoiceGateway(const std::string& token, const std::string& session_id, const std::string& guild_id, const std::string& user_id) : Gateway(token) {
