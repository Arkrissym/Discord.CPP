...
void DiscordCPP::VoiceGateway::on_websocket_incoming_message(web::json::value payload) {
	...
	switch (op) {
		...
		case 6:
			...
			break;
		case 8:
			_heartbeat_interval = (int)(payload[U("d")][U("heartbeat_interval")].as_integer() * 0.75);
			identify();
			break;
		case 9:
			_reconnect_timeout = 0;
...
DiscordCPP::VoiceGateway::VoiceGateway(std::string token, std::string session_id, std::string guild_id, std::string user_id) : Gateway(token) {
